import {
  formatEther,
  isAddress
} from "./chunk-DOPX542K.js";
import "./chunk-PR4QN5HX.js";

// src/actions/checkRewardAction.ts
import { elizaLogger } from "@elizaos/core";
import { checkColdWalletReward } from "mind-randgen-sdk";
var checkRewardAction = {
...
// src/actions/encryptAction.ts
import { ModelClass, elizaLogger as elizaLogger2, composeContext, generateObject } from "@elizaos/core";
...
var encryptAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger2.log("Starting Mind Network MIND_FHE_ENCRYPT handler...");
    const resolvedState = state ? await runtime.updateRecentMessageState(state) : await runtime.composeState(message);
    const dataContext = composeContext({
      state: resolvedState,
      template: dataExtractionTemplate
    });
    elizaLogger2.log("Data context:", dataContext);
    ...
    try {
      ...
      elizaLogger2.error("Error during FHE encryption:", error);
      if (callback) {
        callback({
...
var submitVoteAction = {
  ...
  handler: async (_runtime, _message, _state, _options, callback) => {
    elizaLogger3.log("Starting Mind Network MIND_FHE_VOTE handler...");
    if (!cache_default.latestEncryptedNumber) {
      const reply = "You need to encrypt a number of your choice first. Tell me your number of choice for FHE encryption.";
      elizaLogger3.success(reply);
      if (callback) {
        callback({
          text: reply,
          content: {}
    ...
    const voteInterval = Math.floor((Date.now() - cache_default.lastVoteTs) / 1e3);
    ...
    try {
      await submitVote(cache_default.latestEncryptedNumber);
      cache_default.lastVoteTs = Date.now();
      ...
      elizaLogger3.error("Error during voting:", error);
      if (callback) {
        callback({
...
var registerAction = {
  ...
  handler: async (_runtime, _message, _state, _options, callback) => {
    elizaLogger4.log("Starting Mind Network MIND_REGISTER_VOTER handler...");
    try {
      await registerVoter();
      const reply = "You have registered successfully.";
      elizaLogger4.success(reply);
      if (callback) {
        callback({
          text: reply,
          content: {}
      ...
      elizaLogger4.error("Error during voter registration:", error);
      if (callback) {
        callback({
...
var index_default = mindNetworkPlugin;
export {
